...
class YamlErrorType(Enum):
    ...
    INVALID_EXPRESSION = auto()  # Invalid expression in YAML data
...
class YamlError(Exception):
    ...
    def __init__(
        self, error_type: YamlErrorType, message: str, path: Optional[str] = None
    ):
...
class YamlStructureError(YamlError):
    ...
    def circular_reference(cls, path: str) -> "YamlStructureError":
        ...
        return cls(
            YamlErrorType.CIRCULAR_REFERENCE, f"Circular reference detected", path
        )
    ...
    def max_depth_exceeded(cls, depth: int, path: str) -> "YamlStructureError":
        ...
        return cls(
            YamlErrorType.MAX_DEPTH_EXCEEDED,
    ...
    def invalid_children(
        cls, details: str, path: Optional[str] = None
    ) -> "YamlStructureError":
        """Create error for invalid children specification"""
        return cls(
            YamlErrorType.INVALID_CHILDREN,
            f"Invalid children path specification: {details}",
            path,
        )

    @classmethod
    def invalid_expression(
        cls, expression: str, path: Optional[str] = None
    ) -> "YamlStructureError":
        """Create error for invalid expression in YAML data"""
        return cls(
            YamlErrorType.INVALID_EXPRESSION,
            f"Invalid expression '{expression}' in YAML data",
            path,
        )